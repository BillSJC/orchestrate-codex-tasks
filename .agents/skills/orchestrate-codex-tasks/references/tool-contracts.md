...
归档能力不属于本 Skill 的工具集合。即使运行时暴露归档工具，也不得调用。
...
Codex 托管 worktree 可能受产品级保留和清理策略影响。“不归档”只约束本 Skill 的归档行为，不保证底层 worktree 永久存在。标记 `✅` 前，确保代码已 Handoff、已用用户认可方式持久化，或用户明确接受暂留 worktree。
